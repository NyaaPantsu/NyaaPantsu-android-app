 ...
 */
class RecentListAdapter(var context: Context, torrentList: LinkedList<Torrent>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View? {
        val view: View?
        val vh: RecentListAdapterRowHolder
        if (convertView == null) {
            view = this.mInflator.inflate(R.layout.list_row, parent, false)
            vh = RecentListAdapterRowHolder(view)
            view.tag = vh
        } else {
            view = convertView
            vh = view.tag as RecentListAdapterRowHolder
        }
...
class RecentListAdapterRowHolder(row: View?) {
